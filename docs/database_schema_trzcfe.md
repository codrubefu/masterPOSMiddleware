...
Database table: `trzcfe`

## Field Definitions

| Field Name | Data Type | Status | Description |
|------------|-----------|--------|-------------|
| `idfirma` | numeric(18, 0) | ✓ Checked | idfirma din tabela company |
| `idcl` | numeric(18, 0) | ✓ Checked | idcl din tabela client sau 0? |
| `stotalron` | numeric(8, 2) | ✓ Checked | subtotalul (suma produselor cu TVA) |
| `redabs` | numeric(8, 2) | ✓ Checked | reducere absoluta, daca nu are se pune 0.00 |
| `redproc` | numeric(5, 2) | ✓ Checked | reducere procentuala, daca nu are se pune 0.00 |
| `itotalron` | numeric(8, 2) | ✓ Checked | incasat total ron = subtotalul |
| `itotaleur` | numeric(8, 2) | ✓ Checked | NULL |
| `itotalusd` | numeric(8, 2) | ✓ Checked | NULL |
...
- **Foreign Keys**:
  - `idfirma` → `company.idfirma`
  - `idcl` → `client.idcl` (or 0)
  - `idtrzcf` → `trzcf.nrbonfint` (after end-of-day closure)
  - `casa` → `gestcasa.idcasa`

## Default Values

- `redabs`: 0.00 (if no discount)
- `redproc`: 0.00 (if no discount)
- `nrdispliv`: 0
- `idlogin`: 0
